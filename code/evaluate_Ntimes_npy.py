...
if __name__ == '__main__':
    ...
    for fold in range(1, args.folds+1):
        ...
        for idx, data_id in enumerate(tqdm(ids_list)):
            ...
            # ground truth comes from the prepared npy labels (same layout used for training)
            # instead of the original Datasets/*/labelsTr nifti files, which are not available.
            label = np.load(os.path.join(config.save_dir, 'npy', f'{data_id}_label.npy'))
            label = label.astype(np.int8)
            dd, ww, hh = label.shape
            label = torch.FloatTensor(label).unsqueeze(0).unsqueeze(0)


            # Predictions are saved at the same spatial shape as the input npy.
            # No resize needed for either AMOS (256,256,192) or Synapse (80,160,160).
